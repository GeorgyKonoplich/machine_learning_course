...
public class DecisionTree {
	...
	private static SplitResult findBestSplit(
		...
		for (int i = 0; i < 10000; i++) features.add(i);
		Collections.shuffle(features);

		for (Item baseItem : new LinkedList<Item>(items)) {

			Object[] feat = baseItem.getAttributeNames().toArray();
			for (int i = 0; i < 100; i++){
				String attr = (String) feat[features.get(i)];
	...
	private static double entropy(Collection<Integer> values) {
		...
		for (Integer count : values) {
			...
			entropy += -1 * p * Math.log(p);//entropy
			//entropy += p * (1 - p);
